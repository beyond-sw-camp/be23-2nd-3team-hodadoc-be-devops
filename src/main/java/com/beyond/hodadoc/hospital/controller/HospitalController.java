...
public class HospitalController {
    ...
    @DeleteMapping("/{id}")
    @PreAuthorize("hasRole('HOSPITAL_ADMIN')")
    public ResponseEntity<?> delete(@PathVariable Long id, @AuthenticationPrincipal Long principal) {
    ...
    // [추가] 내 병원 진료과 목록 조회 - 의사 등록/수정 폼에서 병원 등록 과목만 보여주기 위함
    // HospitalDetailDto 구조 변경 없이 별도 API로 분리
    @GetMapping("/my/departments")
    @PreAuthorize("hasRole('HOSPITAL_ADMIN')")
    public ResponseEntity<?> getMyHospitalDepartments(@AuthenticationPrincipal Long principal) {
        List<Map<String, Object>> departments = hospitalService.getMyHospitalDepartments(principal);
    ...
    @PreAuthorize("hasRole('HOSPITAL_ADMIN')")
    public ResponseEntity<?> updateApprovalMode(@PathVariable Long hospitalId,
    ...
    @PreAuthorize("hasRole('HOSPITAL_ADMIN')")
    public ResponseEntity<?> toggleCheckinClose(@PathVariable Long hospitalId,
