...
<details>
<summary><strong>배포</strong></summary>

<details>
<summary>1. 배포 후 병원 상세 페이지 카카오맵 미표시</summary>

### 🔴 문제 상황
로컬 개발 환경에서는 병원 상세 페이지와 병원 등록 페이지에서 카카오맵이 정상 표시되었으나, 운영 서버 배포 후 지도 영역이 완전히 비어있는 현상 발생. 또한 DB에 저장된 병원의 위도/경도 값이 모두 `0`으로 저장되어 있었음

### 🔍 원인

| 단계 | 내용 |
|------|------|
| 1차 확인 | 카카오맵 도메인 등록 여부 확인 → 환자 페이지 도메인만 등록, 병원관리자 페이지 도메인 누락 |
| 2차 확인 | 브라우저 콘솔 확인 → `data.x: undefined`, `data.y: undefined` 출력 |
| 3차 확인 | Network 탭에서 카카오 SDK 요청 확인 → `401 Unauthorized` |
| 근본 원인 | 병원관리자 페이지 도메인 미등록으로 인한 SDK 로드 실패 |

프론트엔드 서버가 환자 페이지와 병원관리자 페이지 두 개로 분리되어 있었는데, 카카오 개발자 콘솔에 환자 페이지 도메인만 등록하고 병원관리자 페이지 도메인 등록을 누락하였다. 이로 인해 병원관리자 페이지에서 카카오 Maps SDK 로드가 `401 Unauthorized`로 실패하였고, 주소 → 좌표 변환이 동작하지 않아 위도/경도가 `0`으로 저장되었다.

### ✅ 해결
카카오 개발자 콘솔에 병원관리자 페이지 도메인을 추가 등록
